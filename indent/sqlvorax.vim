...
" Notes:
"    This indent file is based on the sqlanyware.vim script provided by
"    David Fishburn <dfishburn dot vim at gmail dot com>
"
"    For reference, please see:
"    http://vim.sourceforge.net/script.php?script_id=495
"
"    My contribution was to:
"      * indent lines after: DECLARE, AS|IS keywords
"      * align the BEGIN with the previous DECLARE if there's any
"      * align the EXCEPTION keyword with the previous BEGIN block
"      * indent WHEN <exception> THEN with the corresponding EXCEPTION
...
setlocal indentkeys+==~end,=~else,=~elseif,=~elsif,=~declare,=~begin,=~exception,0=~when,0=)

" GetSQLIndent is executed whenever one of the expressions
" in the indentkeys is typed
setlocal indentexpr=GetVoraxSQLIndent()

" Only define the functions once.
if exists("*GetVoraxSQLIndent")
  finish
endif
let s:keepcpo= &cpo
set cpo&vim
...
function! s:GetStmtStarterIndent( keyword, curr_lnum )
  " echom 's:GetStmtStarterIndent: keyword='.string(a:keyword).' curr_lnum='.a:curr_lnum
  ...
  elseif a:keyword =~? 'begin'
    ...
    let stmts = '^\s*\('.
          ...
          \ '\<create\>' .
          \ '\)'
    let matching_lnum = searchpair(stmts, '', '\<begin\>', 'bW',
          \ 's:IsColComment(line("."), col(".")) == 1')
    exec 'normal! $'
    if matching_lnum > 0 && matching_lnum < a:curr_lnum
      if getline(matching_lnum) =~ '^\s*\<begin\>'
        let ind = -1
      else
        let ind = indent(matching_lnum)
      endif
  ...
  elseif a:keyword =~? 'when'
    ...
    exec 'normal! $'
    if matching_lnum > 0 && matching_lnum < a:curr_lnum
      if getline(matching_lnum) =~ '^\s*\<exception\>'
        let ind = indent(matching_lnum) + &sw
      else
        let ind = indent(matching_lnum)
      endif
...
function! GetVoraxSQLIndent()
  ...
  elseif line =~? '^\s*begin\>'
    let ind = s:GetStmtStarterIndent('begin', v:lnum)
  elseif line =~? '^\s*exception\>'
    let ind = s:GetStmtStarterIndent('exception', v:lnum)
  elseif line =~? '^\s*end\>'
